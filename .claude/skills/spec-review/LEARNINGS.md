...
- **When a spec extends an existing subsystem that wraps a vendor/provider, audit the INHERITED premise, not just the new build.** Observed at scale: ~35k LOC of lifecycle orchestration grown around two *real* provider gaps, on a founding latency figure that was a timeout constant (measured reality: sub-3 s) and a cost figure modeled but never invoiced — while dozens of symptom-scoped fixes landed inside the premise and every review gate only ever audited *new* capability. The provider-fit lane's PF-7 tripwires exist for this: fix-cluster history ≥3 in the subsystem, management-to-workload LOC ratio, invented vocabulary absent from the vendor's docs, unmeasured premise numbers, the vendor's canonical primitive defined with zero call sites. One field survey ("how do 3+ providers in this class serve this use-case?") exposes the paradigm mismatch that years of per-incident analysis structurally cannot.
