...
phase('RCA')
const rcas = await parallel(INCIDENTS.map(i=>()=>ctxAgent(
...
// opus: cross-RCA synthesis into shared-mechanism failure classes — judgment across the whole
// batch, not a per-incident pass.
const tax = await ctxAgent(`Cluster these RCAs into failure CLASSES by shared root MECHANISM (not symptom). Surface cross-cutting: (1) hot-zone files — a few high-blast files own most incidents; (2) uncoordinated parallel sessions on the same substrate; (3) where local-green is necessary-but-insufficient; (4) repeated-fix chains. RCAs: ${JSON.stringify(rcas.filter(Boolean))}`, {label:'synthesize', phase:'Synthesize', schema:TAX, model:'opus', effort:'high', agentType:'general-purpose'})
...
const liveness = await parallel(liveClaims.map(r=>()=>ctxAgent(
