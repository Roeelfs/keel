...
// Mix both runtimes into the buckets, write them to a buckets.json ([[path,...], ...]), then
// PRE-EXTRACT before launching this Workflow — a raw window can be hundreds of transcripts /
// several GB, unreadable by five miners in one turn (measured: 378 transcripts / 2.7 GB):
//   python3 .claude/skills/improve-harness/scripts/digest.py --window <label> buckets.json <outdir>
// This writes one <outdir>/bucket-<i>.md digest per bucket (human turns, tool errors, hook
// denials, REPEATED CALLS for friction loops, self-flags — both runtimes; 176-327 KB measured).
// Pass the digest paths in, one per bucket, in the SAME order as the buckets.
...
const DIGESTS = ARGS.digestPaths // [path, ...] — one pre-extracted bucket-<i>.md digest per bucket, written by scripts/digest.py before launch
...
phase('Mine')
const minings = await parallel(DIGESTS.map((d,i)=>()=>ctxAgent(
  `Mine this pre-extracted digest for harness lessons. Read the digest file at ${d} — scripts/digest.py already surfaced, per session: HUMAN turns, TOOL ERRORS, HOOK DENIALS, REPEATED CALLS (an identical tool call repeated >=3x — a friction-loop candidate), SELF-FLAGS. Cover BOTH runtimes; the digest normalizes Claude JSONL and Codex rollout JSONL into the same sections. Extract THREE things: (1) human-typed turns + corrections; (2) the assistant's own self-flagged mistakes; (3) RECURRING FRICTION LOOPS — the same operation retried ≥3× to no effect, repeated identical errors, repeated permission denials, a merge/push/CI step attempted-and-blocked over and over. Friction loops are usually SILENT (the agent never flags them) — the digest's REPEATED CALLS lines are exactly this signal, already computed; do not re-derive it by hand. Rank by impact — a recurring friction loop ranks at the TOP, it is the highest-signal lesson there is. Quote evidence with the session path the digest cites (include the repeated-call count). Only open a raw JSONL by path — quoted next to each session's heading in the digest — if you need more context for a quote (parse with python, never grep). Set change_kind per lesson: a friction loop that is mechanically detectable at a tool boundary MUST be change_kind:settings-hook (a PreToolUse/Stop guard that blocks the wrong move) — never demote a mechanical block to a prose rule the model "should remember" (a remembered rule is exactly what failed in the stuck-on-merge case). Return report_markdown + lessons[].`,
...
const surveys = await parallel([
  ...
  ()=>ctxAgent(`Audit the PREVIOUS /improve-harness run — only the previous one, four mechanical checks, no re-mining:
...
const verified = (await parallel(claims.map((c,i)=>()=>ctxAgent(
...
// opus: the terminal consolidation into ONE sequenced plan — sequencing, hook-vs-rule routing, and
// what to drop are judgment calls over the whole run's survivors, not another mining pass.
return await ctxAgent(
