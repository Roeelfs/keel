...
3. **Fix clustering** — this is the **≥3rd fix in the same subsystem** (Phase 1's count). Each prior fix was symptom-scoped; none re-questioned the premise. Run this phase at the subsystem level — *"should this machinery exist at all?"* — before landing fix N. **The clustering signal forks into two exits, and Phase 5 must pick one explicitly — not drift into a 3rd surface patch:** either (A) the machinery shouldn't exist (adopt/delete, per the tripwires above), *or* (B) it's legitimate but every prior fix patched a **surface above a shared chokepoint** — so fix N belongs at that chokepoint and must **fail closed on the ambiguity the surface patches kept papering over**, never at surface N. A 3rd patch of one failure class at a 3rd call site is the tell that the resolver / gate / dispatch point *below* all of them is the real fix site: patch it once there and the class dies for every surface at once; patch surface N and it recurs at the next un-covered call site. (Exit B is a fix-**placement** error, not a build-vs-adopt one — the chokepoint case is legitimate machinery fixed at the wrong layer, which is why the premise re-audit alone doesn't catch it.)
...
**One structural blind spot to state out loud when you rely on the occurrence rollup: a failure-keyed GROUP BY (grouping on error class / any non-`success` terminal status) is invisible to a *silent-wrong-success* defect** — one that terminates `status=success` but with a **wrong value** (wrong recipient, wrong sending identity, wrong amount, or a row silently made unreachable). It emits no error to group on, so the recurrence rollup reads **zero** and the class recurs undetected — which is exactly how a mis-routing bug can be patched three times without any telemetry ever surfacing it. This class is caught only by a **count/parity probe** (compare the rows in a terminal state to the count that *should* be there — the gap is the defect) and prevented only by a **loud fail-closed guard at the chokepoint**, never by the occurrence ledger. When an incident's symptom is "right status, wrong outcome," do not read its absence from the rollup as absence of the defect.
...
7. **Fix clustering fires a premise re-audit** — the ≥3rd fix in one subsystem escalates Phase 5 to the subsystem itself ("should this machinery exist?"); any number justifying the incumbent architecture must trace to a **measurement or invoice** (an unmeasured premise is unfalsifiable by construction), and the field survey names how real peers serve the use-case. When the machinery *is* legitimate, the clustering still means fix N belongs at the **shared chokepoint below the patched surfaces — failing closed on the ambiguity — never at surface N** (a fix-placement error the premise re-audit alone won't catch).
