...
1. **Frame (internal fan-out).** Parallel reader agents map the premise's own reality — what the task needs, what already exists in this codebase/architecture, the real goal and constraints, prior art. Every codebase claim cites a real `file:line`, never a guess; a synthesis step distils them into a Problem Frame plus the sharp research questions that seed phase 2. The frame is what makes the research queries *yours*, not generic. **A rewrite / migration / cutover premise adds a fifth angle — the contract × representation inventory** (copies per contract, counted from the code), and the brief's recommendations must then name one owner per contract, the deletions, and what migrates into the owner first; the `cutover-architect` agent owns the rationale and is the standalone form of the same audit.
