...
const SOURCE_TIERS = {
  ...
  market_real_world: 'https://www.federalregister.gov/api/v1/documents.json?conditions%5Bterm%5D=<term>&per_page=2 | https://efts.sec.gov/LATEST/search-index?q=%22<phrase>%22&forms=10-K (UA REQUIRED) | https://data.sec.gov/submissions/CIK<10-digit>.json (UA REQUIRED, 164KB - take filings.recent) | https://api.openalex.org/works?search=<q>&per-page=2&select=id,display_name,publication_year,cited_by_count (select= turns 66KB into 865B) | https://api.crossref.org/works?query=<q>&rows=2 | https://nominatim.openstreetmap.org/search?q=<q>&format=json&limit=2 (UA REQUIRED, 1 req/s - never fan out) | GOTCHA: GDELT is 429 to a fan-out',
