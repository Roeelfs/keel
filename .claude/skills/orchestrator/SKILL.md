...
**Missions must be SELF-CONTAINED — headless lanes have no authenticated MCP servers.** Interactively-authenticated MCPs (issue trackers, DB consoles, browser extensions) are absent or unauthenticated in a headless non-interactive session. A mission that says "read ticket X via the tracker" runs blind and improvises. The orchestrator (which HAS the MCPs) resolves everything up front and embeds it in the mission file: the issue body, acceptance criteria, spec excerpts, relevant memory refs, file anchors. Static-credential CLIs (`gh`, `git`, cloud CLIs with on-disk profiles) still work in lanes — route any lane-side state reads/writes through those, and let the orchestrator do tracker updates itself when the lane's result returns. For long-lived lanes prefer `claude --bg` and read `claude agents --json` at each wake. Same mission-content requirements as Paste 1 below (goal / scope / constraints / trailer / verify / PR format); the mission must also state its **expected merge tier** and any **`stacked_on: <pr>`** edge.
